...
class MainActivity : AppCompatActivity(), UsernameDialogFragment.UsernameDialogListener, InvitationDialogFragment.InvitationDialogListener {
        ...
        bubble.setOnTouchListener { v, event ->
            when (event.action) {
                ...
                android.view.MotionEvent.ACTION_MOVE -> {
                    ...
                    if (isDragging) { v.x = origX + dx; v.y = origY + dy; true } else false
                }
                android.view.MotionEvent.ACTION_UP -> {
                    if (!isDragging) {
                        val parentW = decor.width
                        val midX    = parentW / 2f
                        v.animate().x(if (v.x + v.width / 2f < midX) 16f else parentW - v.width - 16f)
                            .setDuration(200).start()
                    }
                    false
                }
    ...
    fun updateGroupCallBubbleTimer(durationSec: Int) {
        groupCallBubbleView
            ?.findViewById<android.widget.TextView>(R.id.gcm_bubble_timer)
        ...
        runOnUiThread {
            try {
                ...
                if (existing != null && existing.isAdded) {
                    // Fragment is hidden — show it again
                    fm.beginTransaction()
                        .show(existing)
                        .commitAllowingStateLoss()
                } else {
